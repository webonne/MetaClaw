(() => {
  ...
  const rows = [
    ...
    { id: 'routing', name: '幂等去重与确定性路由', scope: '(system,error_code) · 5 分钟桶', cells: [
      ...
      S('done', '路由与去重已实现', 'InMemorySopRepository 精确查表，API 按 system/error_code/service/时间桶去重。'),
    ...
    { id: 'contract', name: '统一 Diagnosis / Case / Run 合同', scope: 'API-first · IM/Web 共用', cells: [
      ...
      S('partial', '数据仍在内存', '合同可用，但 Case/Run/审计记录尚未落数据库或对接主 MetaClaw 服务。'),
      S('partial', '只有本地兼容验证', '有接口测试和浏览器验收，缺版本迁移、并发与恢复验证。'),
    ] },
    ...
    { id: 'closure', name: '关闭归档与知识候选', scope: 'AuditEvent · KnowledgeCandidate', cells: [
      ...
      S('todo', '未持久化审计', '关闭与候选仍随进程重启清空，也未接知识审核服务。'),
      S('partial', '本地审计链完整', 'API 与浏览器验证通过；缺审计不可抵赖、保留期与检索验证。'),
    ] },
    { id: 'knowledge-ops', name: '知识生命周期与 Owner 运营', scope: '审核 · 晋升 · SLA · KPI', cells: [
      ...
      S('partial', '只做到候选生成', '能生成候选并 fail-closed，尚无 approve/deprecate 服务与变更审计。'),
    ...
    { id: 'platform', name: '生产平台化能力', scope: '持久化 · 权限 · 密钥 · 可观测', cells: [
      S('done', '平台原则已覆盖', '内网部署、凭证不入模型、API-first 与部门级演进方向已写入蓝图。'),
      S('partial', '只有端口和本地模型', '存储、认证、租户、密钥和部署契约尚未形成完整资产。'),
